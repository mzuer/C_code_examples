 ...
 * J'ai essayé de convertir toutes les lettres du tableau 4 en majuscules : d'abord avec une boucle et "toupper" mais
 * évidemment ça ne marchait pas puisque cette fonction travaille sur des caractères individuels et le tableau est en une
 * seule dimension donc impossible de faire 2 boucles imbriquées, idem avec la technique
        if(minuscule) {caractere -= 'a' - 'A'}; else {caractere += 'a' - 'A'};
 * Alors ensuite j'ai essayé la fonction strupr :
        for (i=0;i<10;i++) strupr(tableau4[i]); mais je ne comprends pas pourquoi, ça n'a pas compilé
 * Ne sachant plus quoi faire, j'ai choisi la solution de facilité et utilisé strcasecmp !
...
printf("========");
printf("\nTri par séléction :%d comparaisons et %d échanges\n", COMPARAISON, ECHANGE);
